...
def retrieve_current_month_income_expense(request):
    if request.user.is_authenticated:
        ...
        if post_account is None:
            month_history_records = HistoryRecord.objects.filter(time_of_occurrence__year=year, time_of_occurrence__month=month,
                                                                 ).order_by("time_of_occurrence")
        elif isinstance(post_account, str) and int(post_account) == -1:
            month_history_records = HistoryRecord.objects.filter(time_of_occurrence__year=year, time_of_occurrence__month=month,
                                                                 ).order_by("time_of_occurrence")
        else:
            month_history_records = HistoryRecord.objects.filter(time_of_occurrence__year=year, time_of_occurrence__month=month,
                                                                 account_id=post_account).order_by("time_of_occurrence")
        ...
        return JsonResponse({"days": days,
                             ...
                             "month_avg_expense": round(month_total_expense / expense_times, 2)
                             if expense_times != 0 else 0})
    else:
        return JsonResponse({"error": "unauthenticated"})
...
def retrieve_month_has_data(request):
    if request.user.is_authenticated:
        ...
        account_list = list(set([m['account_id'] for m in hr.values()]))
        all_accounts = {i['id']:i['name'] for i in  list(Account.objects.all().values())}
        account_list = [{'id':_id , 'name':all_accounts[_id] } for _id in account_list]
        return JsonResponse({"months": month_list, 'accounts':account_list})
    else:
        return JsonResponse({"error": "unauthenticated"})
